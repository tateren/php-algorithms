...
it('should create hash table of certain size', function () {
    ...
    expect(count($defaultHashTable->buckets))->toBe(32);

    $biggerHashTable = new HashTable(64);
    expect(count($biggerHashTable->buckets))->toBe(64);
});
...
it('should get all the values from empty hash table', function () {
    $hashTable = new HashTable();
    expect($hashTable->getValues())->toEqual([]);
});

it('should get all the values in case of hash collision', function () {
    $hashTable = new HashTable(3);
